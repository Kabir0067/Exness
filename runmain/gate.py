"""
gate.py - Model quality gate, auto-training, and live retraining cycle.

All functions read/write through ``bootstrap.state`` so that the engine
reference is always the most recently wired one.

Public API
----------
monitoring_only_mode()            -> bool
auto_retrain_enabled()            -> bool
required_gate_assets()            -> tuple[str, ...]
model_gate_ready_effective()      -> (ok, reason, active_assets)
models_ready()                    -> (ok, reason)
prime_engine_model_health()       -> None
auto_train_models_strict()        -> bool
run_retraining_cycle(notifier, reason) -> bool
log_monitoring_only_profile()     -> None
"""
...
_log_gate = setup_named_logger("gate", file_name="gate.log", level=logging.INFO, backups=3)

# ---------------------------------------------------------------------------
# Gate-status deduplication  (log at most once per TTL or on change)
# ---------------------------------------------------------------------------
...
def _gate_assets_tuple(
    ...
    assets: dict[str, Any] = details.get("assets", {}) if isinstance(details, dict) else {}
...
def _model_gate_ready() -> tuple[bool, str]:
    ...
    details = gate_details(required_assets=required_gate_assets(), allow_legacy_fallback=True)
        ...
        for asset, ok, reason, version, sharpe, win_rate, max_dd, legacy in items:
            try:
                _log_gate.info(
                    ...
                    asset, ok, reason, version, sharpe, win_rate, max_dd, legacy,
                )
            except Exception:
                continue
...
# ===========================================================================
# Effective gate (partial + soft fallback logic)
# ===========================================================================
def model_gate_ready_effective() -> tuple[bool, str, list[str]]:
    """
    Returns (ok, reason, active_assets).
    active_assets is the subset of required_gate_assets() that passed.
    """
    ...
    # Tier 3: sample-quality soft fallback
    sq = [
        a
...
# ===========================================================================
# Full model-payload readiness check
# ===========================================================================
def models_ready() -> tuple[bool, str]:
    """
    Verify that all active-gate assets have .pkl + .json payloads on disk.
    Returns (ok, reason).
    """
    try:
...
# ===========================================================================
# Engine model health pre-flight
# ===========================================================================
def prime_engine_model_health() -> None:
    ...
    try:
        ...
        try:
            reason_now = str(
                getattr(engine, "_gate_last_reason", "") or ""
            ).strip().lower()
...
# ===========================================================================
# Auto-train (startup strict mode)
# ===========================================================================
def auto_train_models_strict() -> bool:  # noqa: C901 вЂ” complex by necessity
    """
    Train all required assets, gate-check each one, persist a failure registry.
    Returns True if all required assets pass (or a partial gate is acceptable).
    """
    ...
    assets: list[str] = [a.strip().upper() for a in assets_env.split(",") if a.strip()]
    if not assets:
        assets = list(required_gate_assets())
    for a in required_gate_assets():
        if a not in assets:
            assets.append(a)
    ...
    def _load_failures() -> dict[str, dict[str, Any]]:
        try:
            ...
            return {
                ...
                if (ku := k.upper().strip()) and ku in assets_set and isinstance(v, dict)
            }
        except Exception:
            return {}
    ...
    def _save_failures(data: dict[str, dict[str, Any]]) -> None:
        try:
            ...
            _log.warning("Auto-train(strict) could not persist failure registry: %s", exc)

    failures = _load_failures()

    # ---- fast-mode env overrides -------------------------------------------
    auto_fast = env_truthy("AUTO_TRAIN_FAST_MODE", "1")
    fast_defaults: dict[str, str] = {}
    if auto_fast:
        fast_defaults = {
            "INSTITUTIONAL_FAST_MODE": "1",
            "INSTITUTIONAL_REQUIRE_ALPHA_GATE": os.environ.get("AUTO_TRAIN_REQUIRE_ALPHA_GATE", "0") or "0",
            "TRAIN_MAX_BARS": os.environ.get("AUTO_TRAIN_MAX_BARS", "20000") or "20000",
            "INSTITUTIONAL_MAX_ITERS": os.environ.get("AUTO_TRAIN_MAX_ITERS", "700") or "700",
            "INSTITUTIONAL_EARLY_STOPPING_ROUNDS": os.environ.get("AUTO_TRAIN_EARLY_STOPPING_ROUNDS", "120") or "120",
            "INSTITUTIONAL_CV_MAX_ITERS": os.environ.get("AUTO_TRAIN_CV_MAX_ITERS", "180") or "180",
            "INSTITUTIONAL_CV_MAX_SAMPLES": os.environ.get("AUTO_TRAIN_CV_MAX_SAMPLES", "18000") or "18000",
            "INSTITUTIONAL_TSCV_FOLDS": os.environ.get("AUTO_TRAIN_TSCV_FOLDS", "3") or "3",
            "INSTITUTIONAL_WFA_WINDOWS": os.environ.get("AUTO_TRAIN_WFA_WINDOWS", "3") or "3",
        }
        _log.info(
    ...
    for asset in assets:
        ...
        if failure_cooldown_hours > 0:
            ...
            if ts > 0.0:
                ...
                if age_h < failure_cooldown_hours:
                    ...
                    _log.warning(
                        "Auto-train(strict) SKIP | asset=%s reason=recent_failure "
                        "age=%.2fh < %.2fh remain=%.2fh last=%s",
                        asset, age_h, failure_cooldown_hours, remain_h,
                        str(rec.get("reason", "unknown")) if isinstance(rec, dict) else "unknown",
                    )
                ...
                if st_path.exists():
                    ...
                    if age_h < retry_hours:
                        ...
                        if gate_ok_now or not force_retry_on_recent_fail:
                            _log.info(
                                "Auto-train(strict) SKIP | asset=%s reason=recent_state "
                                "age=%.2fh < %.2fh gate_ok=%s gate_reason=%s",
                                asset, age_h, retry_hours, gate_ok_now, gate_reason_now,
                            )
                            (passed_assets if gate_ok_now else failed_assets).append(asset)
                            continue
                        _log.warning(
                            "Auto-train(strict) FORCE_RETRAIN | asset=%s "
                            "reason=recent_state_but_gate_failed age=%.2fh gate_reason=%s",
                            asset, age_h, gate_reason_now,
                        )
            except Exception:
                pass
        ...
        try:
            ...
            if isinstance(ast, dict) and bool(ast.get("ok", False)):
                _log.info(
                    ...
                    asset, float(ast.get("sharpe", 0.0) or 0.0),
                )
        ...
        # --- force-retrain detection ----------------------------------------
        force_retrain = False
        force_retrain_reason = ""
        try:
            gate_ok_pre, gate_reason_pre = _single_asset_gate_status(asset)
            r = str(gate_reason_pre or "")
            wfa_no_evidence = r.startswith(("state_wfa_failed", "meta_wfa_failed")) and (
                ":0/0" in r or "unknown" in r
            )
        ...
        if force_retrain:
            ...
            _log.warning(
                ...
                asset, force_retrain_reason or "gate_failed",
            )
        ...
        try:
            ...
            if gate_ok_asset:
                ...
                _log.info(
                    "Auto-train(strict) passed | asset=%s sharpe=%.3f win_rate=%.3f "
                    "max_dd=%.3f gate_reason=%s",
                    asset,
                ...
                _log.warning(
                    "Auto-train(strict) failed gate | asset=%s reason=%s "
                    "sharpe=%.3f win_rate=%.3f max_dd=%.3f",
                    asset, gate_reason_asset,
                    float(getattr(metrics, "sharpe_ratio", 0.0) or 0.0),
                    float(getattr(metrics, "win_rate", 0.0) or 0.0),
                    float(getattr(metrics, "max_drawdown_pct", 0.0) or 0.0),
                )
    ...
    if failed_assets:
        _log.error("AUTO_TRAIN_QUALITY_FAILED | failed_assets=%s", ",".join(failed_assets))
...
# ===========================================================================
# Live retraining (triggered by supervisor)
# ===========================================================================
...
def _restore_model_artifacts(
    backup_dir: Optional[Path], models_dir: Path
) -> None:
...
def _safe_retrain_models(notifier: Any) -> bool:
    ...
    try:
        assets_env = "XAU,BTC"
        assets = [a.strip().upper() for a in assets_env.split(",") if a.strip()] or list(required)
        for a in required:
            if a not in assets:
                assets.append(a)
...
def run_retraining_cycle(notifier: Any, *, reason: str) -> bool:
    """
    Orchestrate a full supervised retraining cycle:
    pause trading в†’ retrain в†’ gate-check в†’ reload model в†’ resume.
    Enforces a per-session ceiling (RETRAIN_SESSION_MAX).
    """
