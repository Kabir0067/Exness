...
from .models import EngineCycleContext, EngineState, StepDecision
...
from core.signal_engine import MLSignal, fetch_ml_payloads, infer_from_payloads
...
class EngineFSMStageServices(IFSMEnginePorts):
    ...
    def step_data_sync(self, state: EngineState, ctx: EngineCycleContext) -> StepDecision:
        ...
        if self._e.manual_stop_active():
            log_health.info("FSM_MONITORING_CYCLE | reason=manual_stop_active execution_disabled=True")
        ...
        if self._e._live_trading_pause_reason(force=False):
            log_health.warning("FSM_LIVE_EVIDENCE_PAUSE | execution_disabled=True analytics_alive=True")
    ...
    def _emit_analysis_signal(self, asset: str, sig: MLSignal) -> None:
        ...
        last_id, last_sig = self._e._analysis_signal_last_notified.get(asset_u, ("", "Neutral"))
        ...
        blocked_reason = ""
        if self._e.manual_stop_active():
            blocked_reason = str(self._e._unsafe_account_reason or self._e._portfolio_stop_reason or "manual_stop_active")
        ...
        try:
            ...
            log_err.error("ANALYSIS_SIGNAL_NOTIFY_ERROR | asset=%s signal_id=%s err=%s", asset_u, signal_id, exc)
    ...
    def step_execution_queue(self, state: EngineState, ctx: EngineCycleContext) -> StepDecision:
        ...
        if halt_reason:
            ...
            nxt = self._e._transition_state(state, EngineState.VERIFICATION, "risk_block_monitoring")
...
from Backtest.engine import BacktestEngine, BTC_BACKTEST_CONFIG, XAU_BACKTEST_CONFIG
from core.risk_engine import AssetExposure
from mt5_client import MT5_LOCK, mt5_status

from .inference import InferenceEngine
from .models import _DIAG_ENABLED, _DIAG_EVERY_SEC, PortfolioStatus, log_diag, safe_json_dumps
...
class EngineRuntimeMixin:
    ...
    def _update_live_evidence(self, *, force: bool = False) -> Dict[str, Any]:
        ...
        # PATCH-2: Zero-signal alarm вЂ” if engine has been running > 4h with 0 signals,
        ...
        try:
            ...
            if unsafe_reason:
                if should_log_unsafe:
                    issue_sample = list(account_state.get("position_issues", []) or [])[:5]
                    grace_sample = list(account_state.get("grace_positions", []) or [])[:5]
                    log_err.critical(
    ...
    def _inspect_live_account_positions(self, positions: Optional[List[Any]] = None) -> Dict[str, Any]:
        ...
        zero_protection_positions = 0
        now_ts = time.time()
        grace_sec = max(0.0, float(getattr(self, "_unsafe_position_grace_sec", 20.0) or 20.0))
        known_symbols = {
