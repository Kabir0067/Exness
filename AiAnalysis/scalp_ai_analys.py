...
def _confirm_direction(
    *,
    ...
    backups = [
        row for row in SCALPING_PROVIDERS
...
def _advanced_bonus(direction: str, market_data: Dict[str, Any]) -> Tuple[float, List[str]]:
...
def _heuristic_reason(direction: str, market_data: Dict[str, Any], notes: List[str]) -> str:
...
def _heuristic_signal(market_data: Dict[str, Any], asset: str) -> Dict[str, Any]:
    ...
    if not any((m1_close, m5_close, m15_close)):
        return {
            ...
            "provider_display": "Local Heuristic", "model": "heuristic_scalp_v3",
        }
    ...
    def smc_bonus(direction: str) -> float:
        ...
        if close5 > 0:
            for ob in obs:
                ...
                if direction == "BUY"  and ob_dir == "bullish" and ob_bot <= close5 <= ob_top:
                    bonus += 0.10
                    break
                if direction == "SELL" and ob_dir == "bearish" and ob_bot <= close5 <= ob_top:
                    bonus += 0.10
                    break

        # BOS
        bos = struct.get("last_bos") or ""
        if direction == "BUY"  and bos == "bullish_bos": bonus += 0.05
        if direction == "SELL" and bos == "bearish_bos": bonus += 0.05
    ...
    def score(direction: str) -> float:
        total = 0.0
        if direction == "BUY":
            if m15_close > ema20_15 > ema50_15 > ema200_15: total += 0.35
            if m5_close > ema20_5:   total += 0.20
            if m5_close > vwap5:     total += 0.15
            if rsi1 >= 45:           total += 0.10
            if rsi5 <= 55:           total += 0.10
            if news_bias == "bullish":             total += 0.05
            if news_bias == "bearish" and high_impact > 0: total -= 0.15
        else:
            if m15_close < ema20_15 < ema50_15 < ema200_15: total += 0.35
            if m5_close < ema20_5:   total += 0.20
            if m5_close < vwap5:     total += 0.15
            if rsi1 <= 55:           total += 0.10
            if rsi5 >= 45:           total += 0.10
            if news_bias == "bearish":              total += 0.05
            if news_bias == "bullish" and high_impact > 0: total -= 0.15
        if age_sec > 90: total -= 0.20
    ...
    if buy_score >= sell_score and buy_score >= AI_MIN_CONF:
        return {
            "signal": "BUY", "confidence": round(buy_score, 2), "entry": float(entry),
    ...
    if sell_score > buy_score and sell_score >= AI_MIN_CONF:
        return {
            "signal": "SELL", "confidence": round(sell_score, 2), "entry": float(entry),
            "stop_loss": None, "take_profit": None,
    ...
    return {
        ...
        "entry": float(entry), "stop_loss": None, "take_profit": None,
...
def analyse(asset: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if signal == "BUY"  and news_bias == "bearish" and high_impact > 0:
        ...
        result["reason"] = f"{result.get('reason', '')} | Хабарҳои фаврии манфӣ BUY-ро мусдуд кард."
    if signal == "SELL" and news_bias == "bullish" and high_impact > 0:
        signal     = "HOLD"
        confidence = min(confidence, 0.74)
        result["reason"] = f"{result.get('reason', '')} | Хабарҳои мусбат SELL-ро мусдуд кард."
