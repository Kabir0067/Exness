...
from .logging_ import _FILE_LOCK, LOG_DIR, log_risk, RiskLogger
from .broker_adapter import BrokerAdapter
from .models import PlanOrderResult, RiskDecision, SignalSurvivalState
from .utils import (
    _SIDE_MAP,
    _STEP_BALANCE,
    _STEP_LOT,
...
class RiskManager:
    ...
    def _reset_daily_state(self) -> None:
        ...
        try:
            ...
            self.daily_peak_equity = float(max(eq, self.daily_start_balance, get_peak_balance(asset="XAU") or 0.0))
    ...
    def _ensure_ready(self) -> bool:
        return self.broker.ensure_ready()

    def _symbol_meta(self) -> bool:
        # backward compat: update local point cache if needed, though better to use broker directly
        if self.broker.ensure_ready():
            self._point = self.broker.point_value()
            return True
        return False

    def _min_stop_distance(self) -> float:
        return self.broker.min_stop_distance()

    def _normalize_price(self, price: float) -> float:
        return self.broker.normalize_price(price)

    def _normalize_volume_floor(self, vol: float) -> float:
        return self.broker.normalize_volume_floor(vol)

    def _tp_usd_to_price(self, entry: float, side: str, volume: float, usd_profit: float) -> Optional[float]:
        return self.broker.tp_usd_to_price(entry, side, volume, usd_profit)

    def _fallback_volume(self) -> float:
        """
        No static lot fallback for adaptive sizing.
        """
        return 0.0
    ...
    def _current_spread_points(self) -> float:
        return self.broker.current_spread_points()
        ...
        *,
        ...
        try:
            ...
            if self.current_phase == "C" and not bool(getattr(self.cfg, "ignore_daily_stop_for_trading", False)):
                reasons.append(f"risk:{self._phase_reason_last or 'phase_c_protect'}")
            ...
            if not bool(getattr(self.cfg, "ignore_sessions", False)) and not in_session:
                reasons.append("risk:session_closed")
            ...
            if spread_pct > float(self.sp.spread_limit_pct) * 1.5:
                reasons.append("risk:excessive_spread")
            ...
            return RiskDecision(allowed=False, reasons=["risk:guard_error"])
        ...
        try:
            ...
            if phase == "A":
                if conf < float(self.cfg.min_confidence_signal):
                    reasons.append(f"risk:low_confidence:{conf:.2f}<{self.cfg.min_confidence_signal}")
            elif phase == "B":
                ultra_min = float(self.cfg.ultra_confidence_min)
                if conf < ultra_min:
                    reasons.append(f"risk:not_ultra_conf_phase_b:{conf:.2f}<{ultra_min:.2f}")
    ...
    def _order_calc_profit_money(self, side: str, volume: float, entry: float, price2: float) -> float:
        return self.broker.calc_profit_money(side, volume, entry, price2)

    def _order_calc_margin(self, side: str, volume: float, entry: float) -> float:
        return self.broker.calc_margin(side, volume, entry)
    ...
    def calculate_position_size(
        ...
        try:
            ...
            lot = self._normalize_volume_floor(float(lot))
        ...
        try:
            ...
            if dist_sl > 0:
                ...
                if dist_tp < dist_sl * rr_min:
                    tp = (
                        self._normalize_price(entry_price + dist_sl * rr_min)
                        if side_n == "Buy"
                        else self._normalize_price(entry_price - dist_sl * rr_min)
                    )
            ...
            # --- HARDENING: FINAL SAFEGUARD ---
            if float(sl) <= 0.0 or float(tp) <= 0.0:
                 return PlanOrderResult(ok=False, reason="zero_sl_tp_detected")
            if abs(float(sl) - float(entry_price)) < 1e-9:
                 return PlanOrderResult(ok=False, reason="sl_equals_entry")
            # ----------------------------------
    ...
    def record_execution_metrics(
        ...
        try:
            ...
            # feed exec monitor
            try:
                spread_points = self.broker.current_spread_points()
    ...
    def record_execution_failure(self, order_id: str, enqueue_time: float, send_time: float, reason: str) -> None:
        self.logger.record_failure(order_id, enqueue_time, send_time, reason)
        self.logger.update_execution_analysis({"slippage_points": 0.0}) # dummy update if needed or just skip

    def _update_execution_analysis(self, metrics: dict) -> None:
        # Backward compatibility proxy
        self.logger.update_execution_analysis(metrics)
