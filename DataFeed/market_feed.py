...
from config import EngineConfig, SymbolParams, TF_MAP
...
if not any(isinstance(h, logging.FileHandler) for h in log_feed.handlers):
    fh = logging.FileHandler(str(LOG_DIR / "market_feed.log"), encoding="utf-8", delay=True)
...
class MarketFeed:
    ...
    def tick_stats(self, df_m1: Optional[pd.DataFrame] = None) -> TickStats:
        ...
        try:
            ...
            with self._data_lock:
                if len(self._tick_deque) < 40:
                    return TickStats(ok=False, reason="low_ticks")
